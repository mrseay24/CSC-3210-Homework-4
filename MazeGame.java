...
public class MazeGame {
	...
	private int dimensionX, dimensionY; // dimension of maze
	  ...
	  public MazeGame(int aDimension) {
	      // Initialize
	      this(aDimension, aDimension);
	  }
	  // constructor
	  public MazeGame(int xDimension, int yDimension) {
	  ...
	  public void mazeAttributes() {
	      // default solve top left to bottom right
	      this.mazeAttributes(0, 0, dimensionX - 1, dimensionY -1);
	  }
	  // solve the maze starting from the start state (A-star algorithm)
	  public void mazeAttributes(int startX, int startY, int endX, int endY) {
	  ...
	  public void updateGrid() {
	      char backChar = ' ', wallChar = '%', spaceChar = ' ', foodChar = '.' , trapChar = 'O';
	      // fill background
	      ...
	      for (int x = 0; x < dimensionX; x++) {
	          for (int y = 0; y < dimensionY; y++) {
	              ...
	              if (current.food) {
	                  grid[gridX][gridY] = foodChar;
	                  if (current.isCellBelowNeighbor())
	                      if (getCell(x, y + 1).food) {
	                          grid[gridX][gridY + 1] = foodChar;
	                          ...
	                          grid[gridX - 1][gridY + 1] = backChar;
	                  	      ...
	                  	      grid[gridX][gridY + 1] = trapChar;
                              grid[gridX + 1][gridY + 1] = backChar;
                              grid[gridX - 1][gridY + 1] = backChar;
                          }
	                  if (current.isCellRightNeighbor())
	                      if (getCell(x + 1, y).food) {
	                          grid[gridX + 2][gridY] = foodChar;
	                          grid[gridX + 1][gridY] = foodChar;
	                          grid[gridX + 3][gridY] = foodChar;
	                      } else {
	  ...
	  public static void main(String[] args) {
	      ...
	      System.out.println("Type u, d, l, r to move symbol or q to quit game.");
